...
**Fixed 24.09 in `67cf3e7`.** Measured 24.09 in a private Word (DispatchEx — the user's open Word untouched) on COPIES of `ROIW_submission_revised.docx` (12 empty `w:ins`, 6 empty `w:del`) and `Missing Market 01272020.docx` (one empty equation): Word opened both without error and its own SaveAs wrote every shell back. The first of the three outcomes, so the classes moved to `audit` (`_empty_shells`), still reported, never refused. The blank box INSIDE a surviving equation stays in `lint`; `revision.validate` still fails an empty equation through the accepted view's shell count. `--allow-existing-lint` and `lint_check` stay, for the classes that still refuse.

### ~~S1 — parse_number reads a DECIMAL comma as a thousands separator, and _render_value writes the result back~~ — FIXED 24.09, `fa15ea1`
...
**Fixed 24.09 in `fa15ea1`.** The number decides where it can (`0,31`, `12,5`, `1 234,5` are decimal from the string alone) and the TABLE decides where it cannot: `decimal_mark` takes the majority of the cells that say. Measured before choosing, over 20,422 corpus tables: 119 had an ambiguous number settled by a decimal comma beside it, and the 70 left unsettled that were sampled were all English count tables — so no evidence reads as English and `update` needs no refusal mid-write. `_render_value` writes a cell back in its own marks (`0,31` -> 0.4 is `0,40`); `decimal=` on `parse_number`, `Table.numbers` and `update` states the mark outright.

### ~~S3 — the reject gate prunes ONE side, so a document with a continuationNotice can never pass it~~ — FIXED 24.09, `1bb9c39`
...
**Fixed 24.09 in `1bb9c39`.** One level below the sketch: `footnotes.find_all` knew Word's separator notes by ids 0 and -1 only, and the notice carries id 1. It now also reads `w:type`, so `prune_orphans` never sees the notice and both gates pass an unchanged document with one, footnotes and endnotes alike. The Misconceptions workaround (`revision/do/W2_tracked_build.py`) is that session's to delete; it has the hash.

### ~~S3 — wrap_visible_span refuses an OMML-preceded span with a helper's ValueError, not its own AnchorError~~ — FIXED 24.09, `5397902`
...
**Fixed 24.09 in `5397902`.** A START no `w:r` holds is refused with the module's own AnchorError, quoting the paragraph. The END is left returning on purpose — the link stops at the prose — and `test_a_span_ending_INSIDE_the_maths_past_its_last_run` now says which side is which.

### ~~S3 — three readers of field_spans want the MARKERS and get the runs: a bookmark over the prose, and two false refusals~~ — FIXED 24.09, `5cdd2ee`
...
**Fixed 24.09 in `5cdd2ee`.** New `_xml.isolate_field` splits the run holding each marker at the marker when prose shares it (both halves keep the run's rPr) and returns the run span that then holds only the field. D2 and D3 cut there; `_label_spans_in` asks which field holds a run's TEXT, by the markers. The four callers that need run boundaries are unchanged; four expired claims on the rewritten lines were deleted with it.

### ~~S2 — a Table's continuation claims a FIGURE's number, so the reader clicks a figure and lands on a table~~ — FIXED 24.09, `afcbdc1`
...
**Fixed 24.09 in `afcbdc1`.** The LABEL bound, not a smaller number: no label word may stand inside the window. Measured first, old against new over every paragraph of 2,987 corpus documents: 905 matches kept, 0 gained, 9 lost — all 9 this defect ("Figures 8–9" claimed by Table 9). **Still open from this entry:** the survivor parked on it (`continue` for `break` in `link_more`'s continuation loop) can now be argued equivalent; that bookkeeping was not done in this batch.

### ~~S2 — an EMPTY OMML delimiter reads as a default bracket, so a piecewise function gains one it never had~~ — FIXED 24.09, `cb95ec8`
...
**Fixed 24.09 in `cb95ec8`.** `_mval_or` gives the default for an ABSENT value only, on begChr, endChr and sepChr; an empty separator joins with a space. The accent, n-ary and group-character readers keep `or`: their tables map an empty value to the same answer the default gives.

### ~~S2 — batch._LABEL misses a label whose run carries a second property, 35% of the corpus~~ — FIXED 24.09, `d8ac5ea`
...
**Fixed 24.09 in `d8ac5ea`.** The run's remaining rPr is read up to its OWN close and no further (the `_FIELD_END_RE` shape). The wider pass the entry suggests — every reader that assumes an order — was not made; this fixes the reader it measured.

### ~~S2 — FLDCHAR_RE matches a marker with no closing tag: one walk returns garbage, the other is right by luck~~ — FIXED 18.09, `b8006f0`
<!-- status: fixed -->
...
**Fixed at the carrier**, both walks in one commit, as the entry asked.
`FLDCHAR_RE` now requires the marker's tag to CLOSE; `_xml.fields` drops
the `find(">")` it needed to compensate, and `_cite_repair`'s
`remove_outer_field` takes `field.end` for `past_end`, the match already
ending past the marker.

**And the obvious version of the fix was wrong**, which is the part to
keep. Requiring a `>` is not enough: `[^>]*` simply runs on to the NEXT
element's `>`, so a truncated marker still matches, one tag further
along. It wants `[^<>]`.

It was found the right way round rather than reasoned about — the
obvious change was made, the two tests already seen red were re-run, and
they were STILL red. Had those tests not existed first, the weaker fix
would have shipped looking correct, and the second search for it would
have been much harder than the first.

Seven real marker forms match and both truncated forms are rejected:
Word's own, pandoc's spaced self-close, `w:fldLock`, attributes before
the type, `w:dirty`, and the open-tag form carrying a `w:fldData` child.
Check: `scratchpad\agents\xml_fields\marker_forms.py`.

**One caller moved with it, and three citation tests caught that**
immediately — `remove_outer_field` looked for the marker's `>` itself,
so with `m.end()` already past it the splice ate a `</w:r>`. Anything
landing later that reads a FLDCHAR match's end wants the same one-line
review; `begin.end()`, `sep.end()` and `end.start()` in `_compare_read`
were already right and are now better.

Claims: two deleted with the line they stood on, three re-argued from
the new match end. `verify_equivalents _xml.py _compare_read.py` — the
full apply-and-run, not the anchor gate — exits 0 over 41 and 29 claims
respectively.

Found by mut-xml-fields, confirmed in the second module by
mut-compare-read, fixed by mut-xml-fields.
...
**Fixed 17.09 in `06af3d2`: the second option, `revision withdraw`.**
`revision.withdraw(paper, why=)` / `docxkit revision withdraw --why TEXT`
puts `prev`'s bytes back on the manuscript, and refuses unless the
manuscript is byte for byte the newest kept redline (opened and saved
since is the author's), the proposal provably replaced `prev` (its stamp's
`base_sha256`, or for an unstamped vehicle a rescue holding `prev`'s
bytes), and Word does not hold the file. The redline stays in
`build/redlines/` as the record; `build/batch.docx` is removed when it IS
the withdrawn proposal, because `verdict` would otherwise read the
author's next save as that batch "rejected in full" — a batch rebuilt
before withdrawing stays. The ledger gains a `withdrawn` event with the
hashes and the reason. The workaround above is no longer needed: withdraw,
then ship and promote as usual. Tests: the `WITHDRAWN` / `WITHDRAW` states
in `test_workflow_states.py` (ten) and the CLI tests.
### ~~S2 — `revision promote` refuses on the stamp AFTER it has overwritten the manuscript~~ — FIXED 17.09, `06af3d2`
...
**Fixed 17.09 in `06af3d2`, all three parts of the sketch.** (1)
`guard.describes(out)` answers True / False / None (no readable stamp),
and `promote` refuses False with `StaleBatch`, exit 4, before the rescue,
the redline or the copy — nothing is written, and the refusal names
`docxkit revision restamp --why "…"` for a tool pass and a rebuild for a
Word session. (2) `ValidateReport.stamp_describes_batch` and an
`== stamp ==` warning at the top of `revision validate`; warned, not
failed, because every gate reads the bytes that are there. (3) `docxkit
revision restamp [BATCH] --why TEXT`, which refuses a batch with no stamp
rather than mint a build record. DSI's `relink.py` is unchanged: DSI's
`log.md` already tells the next round to restamp after it, and the
refusal now names the command. Tests:
`test_promote_refuses_a_batch_CHANGED_since_its_stamp_BEFORE_writing`,
`test_describes_says_whether_a_stamp_is_about_THESE_bytes` (six shapes),
the `restamp` and `validate` CLI tests in `test_cli_revision.py`.
...
### ~~S1 — `mask_volatile_fields` masks from the WRONG separator when one field nests inside another's instruction half~~ — FIXED 16.09, `dc8bdc8`
...
### ~~S2 — `out_of_order` raises zip's own error out of `revision status` and `revision build`~~ — FIXED 16.09, `0d1c460`
...
Four defects were found on `Aging_Well` on 29–30 August by using the toolkit
rather than by testing it: `citations` refusing on a Word lock while its five
siblings fell back (fixed, `95024d9`), `smarten --write` dropping its backup
...
347 real manuscripts for bugs that "passed the synthetic suite and failed on
documents nobody wrote the rules for". None of that reached these four,
and the reason is the same each time.

**They are defects of composition, of workflow state, or of prose.**

*Composition.* The lock seam IS tested — `is_locked`, the snapshot fallback,
22 files mention it. What no test owns is whether the six `[verify]` commands
AGREE under a lock. Each command's tests verify that command; nothing verifies
the set. The defect was one refusal among five fallbacks, invisible until
someone ran all six on one held file and compared exit codes. The suite has
exactly one cross-command sweep (`test_cli_guards.py`) and it parametrises
aliases and page ranges, not the command list.

Same shape for `m:sepChr`: the page rendered correctly, the XML was valid, and
FORMULA compared tokens. **Every layer passed its own tests because every
layer was right about its own job.** Nobody asked whether ANY layer could see
the operator.

*Workflow state.* `test_revision_verdict.py` already has two stale-batch tests
— one stamped against a different baseline, one unstamped — and both correctly
answer "cannot tell". The batch that broke it was a third state: **correctly
stamped against the CURRENT baseline, and never promoted.** The guard asks
"was this built from the current baseline?" and answers yes, rightly. The
question that mattered — "did this ever reach the manuscript?" — is asked
nowhere, and `unpromoted` appears in zero test files. That state exists only
because a round was deliberately HELD for an author decision, which is a thing
protocols do and fixtures do not.

*Prose.* `paper.toml` on `Aging_Well` banned U+2032 because `build` once
refused it. The refusal was filed, fixed on 24.08, and
`hygiene.MATH_DOWNGRADES` has carried `′` ever since. The prohibition stayed,
and a round in August drafted itself around a constraint that no longer
existed. **No test can fail on a stale comment.** `tools/backlog_refs.py`
exists for exactly this failure mode and gates THIS file; nothing gates a
per-paper config.

**Two cheap gates would close most of it.**

1. ~~**A cross-command conformance sweep.**~~ **Built 31.08, `eac1053`.** One
   parametrised test over every read-only command, run against a locked file,
   asserting a single contract: either produce a report, or refuse in a way
   distinguishable from a clean run. That one test is the whole of the
   `citations` defect, and it keeps holding as commands are added — which is
   the property the per-command tests cannot have.

   **And that property was the half that was missing.** A sweep existed; its
   command list was 15 tuples written by hand on 2026-08-23, and the CLI had
   25 commands. `link` and `linkfix` were never in it and both still refused
   under a lock — the same defect, in the two commands `citations` sends its
   reader to next. The list is derived from `cli.build_parser()` now (which is
   why `main` no longer builds its own parser), every command is either swept
...
not produce — which is the honest reason they did not exist, and not a
criticism of the suite that caught the stale baseline, the reference ordering,
the empty link husk and the symbol typed as text in the same two days.

**Half of the second one arrived with a fix rather than as a fixture set.**
Closing the `revision baseline` verdict entry (31.08, `39fe472`) needed the
built-and-HELD state — a batch correctly stamped against the current baseline
and never promoted — so `test_revision_verdict.py` now builds and promotes in
...
**Closed 11.09.** Both gates the note asked for are built — the cross-command
sweep on 31.08 (`eac1053`) and the workflow-state fixture set on 11.09
(`396b048`) — so nothing of the proposal remains open. The record stays,
here, because the four defects and the dates they were found on are cited
by this heading.
