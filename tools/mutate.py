...
MUTATIONS = [
    ...
    Mutation("_xml.py", "escaping stops being applied to run text",
             ...
             "    return text"),
    # --- tables ---------------------------------------------------------
    Mutation("tables.py", "nested tables close on the first end tag",
             '        end = matching_close(xml, at + len("<w:tbl>"), "tbl")',
             '        end = xml.index("</w:tbl>", at) + len("</w:tbl>")'),
    Mutation("tables.py", "stale table offsets are used instead of refused",
             "    if table.source is not None and table.source != hash(xml):",
             "    if False:"),
    Mutation("tables.py", "spans constrain by full width (the ~250 dxa bug)",
             "            spans.append((c, k, h))",
             "            spans.append((c, k, f))"),
    Mutation("tables.py", "declared cell margins are ignored",
             "    side = 2 * margin if margin is not None "
             "else _side_margins(body)",
             "    side = 2 * margin if margin is not None else 216"),
    Mutation("tables.py", "the closing rule lands on the first row",
